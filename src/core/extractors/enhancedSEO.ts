...
export interface EnhancedSEOMetadata {
  // Indexability data
  indexability: {
    metaRobots?: string; // e.g., "index, follow" or "noindex"
    xRobotsTag?: string; // From HTTP header
    canonical?: string; // Resolved canonical URL
    isNoIndex: boolean; // Derived from metaRobots or xRobotsTag
    isNoFollow: boolean; // Derived from metaRobots or xRobotsTag
  };
  
  // On-page content data
  content: {
    title?: string;
    titleLength?: { characters: number; pixels: number }; // Approximate pixel width
    metaDescription?: string;
    descriptionLength?: { characters: number; pixels: number };
    h1?: string;
    h1Count: number; // How many H1s on the page
    h2Count: number;
    h3Count: number;
    h4Count: number;
    h5Count: number;
    h6Count: number;
    wordCount: number; // Main content word count
    textContentLength: number; // Total text length in characters
  };
  ...
  international: {
    ...
    hreflangErrors?: string[]; // e.g., "Missing return link from en-US"
  };
  ...
  schema: {
    ...
    schemaTypes: string[]; // e.g., ["Article", "Organization"]
  };
}
...
export interface EnhancedSEOInput {
  ...
  bodyText?: string; // Optional pre-extracted body text for word count
}
 ...
 */
export function extractEnhancedSEOMetadata(input: EnhancedSEOInput): EnhancedSEOMetadata {
 ...
 */
function estimatePixelWidth(text: string, isTitle = true): number {
 ...
 */
function extractSchemaTypes(data: any, types: string[]): void {
