...
export interface StructuredDataItem {
  ...
  schemaType?: string;  // Schema.org type: "Product", "Article", "Organization", etc.
  data: any;            // Raw structured data object
}

export interface StructuredDataInput {
  html: string;
  url: string;       // For resolving relative URLs and context
}
 ...
 */
export function extractStructuredData(input: StructuredDataInput): StructuredDataItem[] {
  const { html } = input;
 ...
 */
function extractMicrodataBasic(html: string): StructuredDataItem[] {
  const results: StructuredDataItem[] = [];
  ...
  try {
    ...
    while ((match = itemTypeRegex.exec(html)) !== null) {
      ...
      // Avoid duplicates
      if (seen.has(schemaType)) continue;
      seen.add(schemaType);
 ...
 */
export function filterRelevantStructuredData(items: StructuredDataItem[]): StructuredDataItem[] {
  ...
  return items.filter(item => {
    if (!item.schemaType) return false;
