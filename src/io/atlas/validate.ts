...
export async function validateAtlas(atlsPath: string, opts?: { checkDuplicates?: boolean, summary?: boolean }): Promise<AtlasValidationResult> {
  const errors: string[] = [];
  const summary: AtlasValidationResult['summary'] = {
      ...
      if (file.endsWith('.jsonl') || file.endsWith('.zst')) {
        ...
        try {
          ...
          if (file.endsWith('.jsonl')) {
            ...
            fs.closeSync(fd);
            if (bytes === 0) errors.push(`Unreadable part file: ${file}`);
          }
        } catch (e) {
  ...
  if (opts?.checkDuplicates) {
    ...
    const urlKeyCounts: Record<string, number> = {};
      ...
      if (file.endsWith('.jsonl')) {
        ...
        for await (const line of rl) {
          ...
          try {
            ...
            if (key) urlKeyCounts[key] = (urlKeyCounts[key] || 0) + 1;
          } catch {}
        }
      }
    }
