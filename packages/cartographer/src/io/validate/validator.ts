...
function datasetHasRecords(manifest: AtlasManifest, datasetName: string): boolean {
  ...
  if (coverageEntry) {
    return coverageEntry.present && coverageEntry.row_count > 0;
  }
...
function validatePackPlan(manifest: AtlasManifest, result: ValidationResult): void {
  if (!manifest.packs || manifest.packs.length === 0) {
    result.packs = { valid: true, summary: [] };
    return;
  }
...
export async function validateAtlas(atlsPath: string): Promise<ValidationResult> {
  ...
  validatePackPlan(manifest, result);
