...
import type { PerformanceSummary } from "../../utils/metrics.js";

export const stressCommand: CommandModule = {
  command: "stress",
  ...
  handler: async (argv) => {
    ...
    if (cfg.reportCsv) {
      ...
      if (fs.existsSync(perfPath)) {
        const perfData: PerformanceSummary = JSON.parse(fs.readFileSync(perfPath, "utf-8"));
 ...
 */
function generateStressReport(perf: PerformanceSummary, csvPath: string): void {
  const rows = [
    "metric,value",
