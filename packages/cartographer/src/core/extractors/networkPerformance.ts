...
export interface NetworkPerformanceMetrics {
  ...
  compression: {
    ...
    deflate: number;
    none: number;
    uncompressibleTypes: number; // Images, fonts, etc.
  };
  
  // Status code summary
  statusCodes: {
    [code: number]: number; // e.g., { 200: 45, 304: 12, 404: 2 }
  };
  ...
  // Resources list (for detailed analysis)
 ...
 */
export function createNetworkPerformanceCollector(page: Page) {
  const resources: NetworkResourceData[] = [];
  ...
  const responseHandler = async (response: Response) => {
    try {
      ...
      let type: NetworkResourceData["type"] = "other";
      ...
      let compression: NetworkResourceData["compression"] = "none";
      if (contentEncoding) {
  ...
  return {
    ...
    getMetrics(): NetworkPerformanceMetrics {
      ...
      const breakdown: NetworkPerformanceMetrics["breakdown"] = {
