 ...
 */
export function extractTwitterCard(html: string): StructuredDataItem | null {
  // Match all meta tags with name="twitter:*"
  const twitterTagPattern = /<meta\s+(?:[^>]*?\s+)?name=["']twitter:([^"']+)["'](?:[^>]*?\s+)?content=["']([^"']*)["'][^>]*>/gi;
  
  const twitterData: Record<string, string | string[]> = {};
 ...
 */
export function extractTwitterAppCard(html: string): StructuredDataItem | null {
 ...
 */
export function extractTwitterPlayerCard(html: string): StructuredDataItem | null {
 ...
 */
export function extractAllTwitterCards(html: string): StructuredDataItem[] {
