 ...
 */
export function countResources(html: string): {
  cssCount: number;
  jsCount: number;
  fontCount: number;
  inlineStyles: number;
 ...
 */
export async function detectFlashingContent(page: any): Promise<{
  hasBlinkTag: boolean;
 ...
 */
export function extractCompression(headers: Record<string, string | string[]>): {
 ...
 */
export function extractViewportMeta(html: string): {
  content: string;
  width?: string;
  initialScale?: number;
  hasViewport: boolean;
 ...
 */
export function checkSubresourceIntegrity(html: string): {
  totalScripts: number;
  totalStyles: number;
  ...
  let totalStyles = 0;
  let stylesWithSRI = 0;
  const missingResources: Array<{ url: string; type: "script" | "stylesheet" }> = [];
 ...
 */
export async function collectAdvancedPerformanceMetrics(page: any): Promise<{
  fid?: number;
  inp?: number;
  speedIndex?: number;
  tti?: number;
    ...
    const metrics = await page.evaluate(() => {
      ...
      try {
        ...
        for (const shift of layoutShifts) {
          if (!(shift as any).hadRecentInput) {
            clsScore += (shift as any).value;
          }
        }
        if (clsScore > 0) {
      ...
      if (performance && performance.getEntriesByType) {
        ...
        const currentOrigin = (globalThis as any).location.origin;
        const renderBlocking: Array<{url: string; type: "script" | "stylesheet"; size?: number}> = [];
 ...
 */
export function countBrokenLinks(edges: Array<{httpStatusAtTo?: number; isExternal: boolean}>): number {
  let count = 0;
 ...
 */
export function extractOutboundDomains(edges: Array<{targetUrl: string; isExternal: boolean}>): string[] {
