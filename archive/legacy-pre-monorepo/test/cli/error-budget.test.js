...
describe("Error Budget Enforcement", () => {
    ...
    it("should stop crawl when error budget is exceeded and return exit code 2", async () => {
        ...
        const hasErrorBudgetNote = json.notes.some((note) => note.includes("error budget exceeded") || note.includes("Terminated"));
    ...
    it("should complete successfully when errors are within budget", async () => {
        ...
        const hasErrorBudgetNote = json.notes.some((note) => note.includes("error budget exceeded"));
...
//# sourceMappingURL=error-budget.test.js.map