...
function isPageFetched(ev) {
    return !!ev
        && typeof ev.type === 'string'
        && ev.type === 'page.fetched'
...
test('Scheduler per-host rate limiting respects perHostRps', async (t) => {
    ...
    const finishedP = new Promise((resolve) => {
    ...
    for (const host of Object.keys(hostEvents)) {
        ...
        for (let i = 1; i < times.length; i++) {
            ...
            assert.ok(dt >= minIntervalMs, `Host ${host} interval ${dt}ms < ${minIntervalMs}ms (perHostRps=${perHostRps})`);
        }
    }
});
//# sourceMappingURL=scheduler.rateLimit.test.js.map