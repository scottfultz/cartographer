...
export class AtlasWriter {
    ...
    constructor(outPath, config, crawlId) {
        ...
        const modeToLevel = { raw: 1, prerender: 2, full: 3 };
    ...
    async writeScreenshot(viewport, urlKey, buffer) {
    ...
    async writeFavicon(originKey, buffer, mimeType) {
    ...
    async writeLegacyScreenshot(urlKey, buffer) {
        const path = join(this.stagingDir, "media", "screenshots", `${urlKey}.png`);
        await writeFile(path, buffer);
    }
    async writeViewport(urlKey, buffer) {
