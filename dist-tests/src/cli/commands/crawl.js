...
function parseParamPolicy(input, fallback = 'keep') {
    if (typeof input === 'string' && PARAM_POLICIES.has(input))
        return input;
    ...
    handler: async (argv) => {
        ...
        const crawlConfig = {
            ...
            discovery: {
                ...
                paramPolicy: parseParamPolicy(argv.paramPolicy),
        ...
        function outStd(msg) { if (!asJson && !quiet)
            process.stdout.write(msg + '\n'); }
        function outErr(msg) { if (!asJson)
        ...
        crawlConfig.discovery.paramPolicy = crawlConfig.discovery.paramPolicy;
        // Import event bus
