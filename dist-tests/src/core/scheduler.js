...
export class Scheduler {
    ...
    faviconCache = new Map(); // origin -> path in archive
    // Public API state
    crawlState = "idle";
    // URL filtering
    urlFilter;
        ...
        try {
            ...
            const pageRecord = {
                ...
                performance: renderResult.modeUsed === "full" && renderResult.performance ? {
                    ...
                    scores: renderResult.performance.lighthouseScores,
                    renderBlockingResources: renderResult.performance.renderBlockingResources,
    ...
    async enqueueIfNew(url, depth, discoveredFrom) {
        ...
        try {
            ...
            const normalizeDomain = (hostname) => hostname.replace(/^www\./i, '');
