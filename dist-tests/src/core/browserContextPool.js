...
export class BrowserContextPool {
    ...
    storageDir;
    contexts = new Map();
    contextUsageCounts = new Map();
    // Context recycling thresholds
    CONTEXT_RECYCLE_THRESHOLD = 50;
    MAX_RSS_MULTIPLIER = 0.7; // Recycle at 70% of max RSS
    constructor(browser, cfg, storageDir) {
     ...
     */
    async saveContext(origin, context) {
     ...
     */
    async saveAllContexts() {
        log('info', `[BrowserContextPool] Saving ${this.contexts.size} context(s)...`);
        const promises = Array.from(this.contexts.entries()).map(([origin, context]) => this.saveContext(origin, context));
        await Promise.all(promises);
    }
    /**
     * Close a specific context
     */
    async closeContext(origin) {
        const context = this.contexts.get(origin);
        if (!context) {
            return;
        }
