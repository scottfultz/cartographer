 ...
 */
export function collectWCAGData($, baseUrl) {
    // === 1.1.1 Images ===
    let imagesTotal = 0;
    let imagesWithAlt = 0;
    ...
    $("audio").slice(0, 20).each((_, audio) => {
        ...
        const hasTranscript = parent.find("a[href*='transcript'], a:contains('Transcript'), a:contains('transcript')").length > 0 ||
    ...
    $("h1, h2, h3, h4, h5, h6").each((_, el) => {
        ...
        if (tagName) {
            headings.push(tagName.toUpperCase());
        }
    });
    ...
    const missingRequiredAriaAttributes = [];
 ...
 */
export async function collectRuntimeWCAGData(page) {
    try {
    ...
    `);
        const runtimeData = await page.evaluate(runtimeScript);
        return runtimeData;
    }
    catch (error) {
 ...
 */
export function extractAriaLiveRegions($) {
    const regions = [];
    // Find explicit aria-live attributes
    $('[aria-live]').each((i, el) => {
        if (i >= 50)
            return false; // Limit to 50 regions
        const $el = $(el);
    ...
    $('[role="status"], [role="alert"], [role="log"]').each((i, el) => {
        ...
        const liveValue = role === 'alert' ? 'assertive' : 'polite';
        const selector = el.tagName.toLowerCase() +
            (el.attribs.id ? `#${el.attribs.id}` : '') +
            (el.attribs.class ? `.${el.attribs.class.split(' ').slice(0, 2).join('.')}` : '');
        regions.push({
 ...
 */
export function analyzeFormAutocomplete($) {
    ...
    personalDataPatterns.forEach(pattern => {
        $(pattern.selector).each((i, el) => {
            if (personalDataInputs.length >= 50)
                return false; // Limit to 50
            const $el = $(el);
            const autocomplete = $el.attr('autocomplete');
            const hasAutocomplete = !!autocomplete;
            const tagName = el.tagName || 'input';
...
//# sourceMappingURL=wcagData.js.map