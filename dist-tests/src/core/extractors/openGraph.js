 ...
 */
export function extractOpenGraphExtensions(html) {
    const extensions = [];
    const namespaces = ['article', 'video', 'music', 'book', 'profile'];
    for (const namespace of namespaces) {
        const pattern = new RegExp(`<meta\\s+(?:[^>]*?\\s+)?property=["']${namespace}:([^"']+)["'](?:[^>]*?\\s+)?content=["']([^"']*)["'][^>]*>`, 'gi');
        const namespaceData = {};
        let match;
 ...
 */
export function extractAllOpenGraph(html) {
    const results = [];
